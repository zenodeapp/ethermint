...
func (mh MultiEvmHooks) PostTxProcessing(ctx sdk.Context, stateDB vm.StateDB, txHash common.Hash, logs []*ethtypes.Log) error {
	for i := range mh {
		if err := mh[i].PostTxProcessing(ctx, stateDB, txHash, logs); err != nil {
